...
// Use this for the Practice Mode version of the test

function buildStudentPracticeHtml(testData, testId) {
...
<style>
/* ===== RESET & BASE (Exact Copy from Sacred Code) ===== */
...
.practice-explanation b { color: #92400e; }
.practice-exp-img { max-width: 100%; max-height: 200px; display: block; margin-top: 10px; border-radius: 8px; border: 1px solid #e2e8f0; }
...
/* ===== RESULT SCREEN (Simplified for Practice) ===== */
#resultScreen {
  background: #f0f4f8;
  align-items: center;
  ...
  <div class="welcome-card">
    ...
    <div class="instructions-box">
      ...
      <ol id="instructionsList">
        <li>This is a Practice Test. You will get <b>instant feedback</b> after every answer.</li>
        <li>Once an option is selected, it cannot be changed.</li>
        <li>Read the explanation provided after answering each question.</li>
        <li>You can still navigate between questions using Next/Previous buttons.</li>
        <li>Click "Finish Practice" at the end to see your total score.</li>
      </ol>
    </div>
...
<div class="modal-overlay" id="confirmModal">
  <div class="modal">
    ...
    <p>Are you sure you want to end your practice session?</p>
...
function showQuestion(index) {
  ...
  opts.forEach(function(opt, displayPos) {
    ...
    // If already answered, apply immediate feedback styles
    if (userAns) {
  ...
  if (userAns) {
    expBox.innerHTML = '<b>💡 Explanation:</b> ' + escHtml(q.explanation || 'No explanation provided.') + 
                       (q.explanationImage ? '<br><img class="practice-exp-img" src="' + convertDriveUrl(q.explanationImage) + '">' : '');
...
function selectAnswer(qIndex, letter) {
  ...
  // Re-render to show colors and explanation immediately
  showQuestion(qIndex);
}
...
function submitTest() {
  ...
  var submitData = {
    ...
    testTitle:    testInfo.title + ' (Practice)',
...
window.addEventListener('beforeunload', function(e) {
  if (testStarted && !testSubmitted) {
    e.preventDefault();
    e.returnValue = 'Practice session chal raha hai! Bahar jaane se progress kho jayega.';
